...
stats). Photo data never leaves the LAN and there are no new cloud accounts, with one deliberate
exception: the weather dashboard source calls the external Open-Meteo API, since a real forecast
needs a data source no amount of self-hosting can replace.
...
- **`photo-frame-bridge/`** - a small self-hosted Flask service with two generic, index-based
  ...
  dashboard sources (weather, PhotoPrism stats). Both endpoints do the source-count wraparound
